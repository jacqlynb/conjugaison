...
function buildConjugationQuery(tense, pronoun, infinitive) {
    ...
    switch (tense) {
        ...
        case 'passé composé':
            return getComposedTenseQuery('indicative_present', pronounType, infinitive);
        case 'plus-que-parfait':
            return getComposedTenseQuery('indicative_imperfect', pronounType, infinitive);
        case 'futur antérieur':
            return getComposedTenseQuery('indicative_future', pronounType, infinitive);
        case 'subjonctif présent': 
            return getSimpleTenseQuery('subjunctive_present', pronounType, infinitive);
        case 'subjonctif passé':
            return getComposedTenseQuery('subjunctive_present', pronounType, infinitive);
        case 'conditionnel présent': 
            return getSimpleTenseQuery('indicative_conditional', pronounType, infinitive);
        case 'conditionnel passé': 
            return getComposedTenseQuery('indicative_conditional', pronounType, infinitive);
        case 'impératif': 
            return getSimpleTenseQuery('imperative', pronounType, infinitive);
        default: throw new Error('invalid tense');
    }
}
...
function getPronounType(pronoun) {
    switch (pronoun) {
        ...
        case 'ils': return 'tpp';
        default: throw new Error('invalid pronoun');
    }
}
