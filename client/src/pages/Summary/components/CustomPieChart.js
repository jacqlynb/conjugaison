...
export function CustomPieChart(props) {
  const data = [
    {name: 'Group A', value: props.numCorrectConjugations},
    {name: 'Group B', value: props.totalConjugations - props.numCorrectConjugations},
  ];
  
  return (
        <PieChart className="custom-pie-chart" width={200} height={200}>
          <text x="50%" y="50%" dy={8} textAnchor="middle">
            {`${props.numCorrectConjugations}/${props.totalConjugations}`}
          </text>
          <Pie data={data} 
               dataKey="value" 
               cx="50%" 
